...
/* A funçao dfsR() visita todos os vértices de G que podem ser alcançados a partir do vértice v sem passar por vértices já descobertos.  (O código supõe que G é representado por uma matriz de adjacências.) */
...
void GRAPHdfs( Graph G,int valor)
{
   for (vertex v =(G->V)-1 ; v >0; --v)
         dfsR( G, v, valor); // começa nova etapa
}
